...
メイン教材は、Windows版Orcaを使って完全なPC初心者が実際に成果物を公開するまでを学ぶコースとする。ブラウザ教材を画面左側、Orcaを右側に置き、説明・クイズ・確認項目を見ながらCodexと共同作業する体験を中心にする。

最初の教材は **ORCA-001（OrcaとCodexで学習を始める）**。以降、練習用プロジェクト作成、Git、GitHub連携、公開へ段階的に進む。Orca固有の画面操作はWindows実機で確認し、確認日と画像を記録できたものだけを掲載する。

## GitHub教材について

既存の **GH-001（GitHubとは何か）** は、Orcaコース後半でGitHubを学ぶための基礎教材として残す。
...
- メインテーマ: Orca（Windows専用）
- メイン教材: `ORCA-001`（OrcaとCodexで学習を始める、`status: draft`）
- 既存教材: `GH-001`（GitHubとは何か、`status: draft`）
- ステータス: ORCA-001初稿とブラウザゲームへの読み込みを実装済み。Orca画面の実機スクリーンショット検証とORCA-002以降は継続作業。
